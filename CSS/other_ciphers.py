...
class Ciphers:
    ...
    def encrypt_vignere(self):
        ...
        Ci = (Pi + Ki) mod 26
        '''
        return ''.join([chr((ord(a) + ord(b)) % 26 + ord('A')) for a,b in zip(self.text, itertools.cycle(self.key))])

    def decrypt_vignere(self, cipher):
        '''
        Pi = (Ci - Ki) mod 26
        '''
        return ''.join([chr((ord(a) - ord(b)) % 26 + ord('A')) for a,b in zip(cipher, itertools.cycle(self.key))])
